...
Usage:
    python3 services/ml/train.py [--epochs N] [--batch-size N] [--lr FLOAT]

The fine-tuned model is saved back to services/ml/models/T5_FineTuned/ when done.
"""
...
MODEL_PATH = REPO_ROOT / "services" / "ml" / "models" / "T5_FineTuned"
CUSTOM_DATA_PATH = REPO_ROOT / "data" / "custom_qa.jsonl"
...
def train(epochs: int, batch_size: int, lr: float):
    ...
    # Build dataset
    custom = load_custom_examples(str(CUSTOM_DATA_PATH))
