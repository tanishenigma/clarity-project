...
Requirements:
    ...
    # Tesseract binary (Linux):   sudo apt install tesseract-ocr
"""
...
try:
    ...
    # Auto-locate Tesseract on Windows without needing PATH changes
    if sys.platform == "win32":
        ...
        if os.environ.get("TESSERACT_PATH"):
            _TESS_CANDIDATES.insert(0, os.environ["TESSERACT_PATH"])

        _found = False
        for _candidate in _TESS_CANDIDATES:
            if os.path.isfile(_candidate):
                pytesseract.pytesseract.tesseract_cmd = _candidate
                print(f"[INFO] Tesseract found at: {_candidate}")
                _found = True
                break

        if not _found:
            print("[WARN] Tesseract binary not found in common Windows paths.")
            print("       Download from: https://github.com/UB-Mannheim/tesseract/wiki")
            print("       Then either:")
            print("         (a) Add install folder to PATH and restart terminal, OR")
            print("         (b) set TESSERACT_PATH=C:\\path\\to\\tesseract.exe")
...
# ---------------------------------------------------------------------------
# Windows long-path helper  (root cause of WinError 2 on deep folder trees)
# ---------------------------------------------------------------------------

def win_long_path(path: Path) -> str:
    """Prepend \\?\ on Windows to bypass the 260-char MAX_PATH limit."""
    if sys.platform == "win32":
...
# ---------------------------------------------------------------------------
# OCR  (PyMuPDF-first to bypass Windows MAX_PATH; pdftoppm as fallback)
# ---------------------------------------------------------------------------

def _rasterise_fitz(pdf_path: Path, dpi: int) -> List["Image.Image"]:
...
def _rasterise_pdftoppm(pdf_path: Path, dpi: int) -> List["Image.Image"]:
