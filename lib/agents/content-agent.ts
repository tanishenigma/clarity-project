...
export class ContentIngestionAgent {
  ...
  private async extractPdfText(url: string): Promise<string> {
    if (!url) {
      console.warn(
        "[ContentAgent/PDF] No URL provided, skipping PDF extraction",
      );
      return "";
    }
    ...
    try {
      ...
      // Lazy-load to avoid @napi-rs/canvas crashing at module init time
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const pdfParse = require("pdf-parse");
      const data = await pdfParse(buffer);
      console.log(
        `[ContentAgent/PDF] Parsed ${data.numpages} page(s), ${data.text.length} chars extracted`,
      );
      return data.text;
    } catch (err) {
  ...
  private async performOCR(url: string): Promise<string> {
    // TODO: Implement OCR using Tesseract or AWS Textract
    console.warn(
      `[ContentAgent/OCR] OCR not implemented yet \u2014 url: ${url}`,
    );
    return "";
  }
